function clearCompanyForm() {
  ...
  companyActive.value = "yes";
}

function saveCompany() {
  const id = companyId.value || uid("co");
  const company = {
    id,
    name: companyName.value.trim(),
    whatsapp: companyWhatsapp.value.trim(),
    active: companyActive.value
  };

  if (!company.name) return alert("Company name required");
...
function editCompany(id) {
  ...
  companyActive.value = company.active;
}
...
function deleteCompany(id) {
  ...
  db.customers.forEach(c => {
    c.companyAccess = (c.companyAccess || []).filter(x => x !== id);
  });
...
function renderCompanies() {
  companyList.innerHTML =
    "<table><tr><th>Company</th><th>WhatsApp</th><th>Status</th><th>Action</th></tr>" +
    db.companies.map(c => `
      <tr>
        <td>${esc(c.name)}</td>
        ...
        <td>${esc(c.active)}</td>
        <td>
          <button class="btn secondary" onclick="editCompany('${c.id}')">✏️ Edit</button>
